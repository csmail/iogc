...
## Odds ratio comparing randomly sampled outlier and non-outlier variant GWAS effect sizes across tissues/genes
...
for (z_thresh in  c(2,4,6)) {
        ...
        ## Subset outliers to intersection
        match_merge_out <- get_out[get_intersection, on=c(tissue="tissue", pheno_id="pheno_id", gene_id="gene_id", exp_match_direction="exp_match_direction")]

        ## Choose one outlier sample randomly
        match_keep_rand_out <- match_merge_out[, .(sample_id=sample(sample_id, 1)),
                by=list(tissue, pheno_id, gene_id, exp_match_direction)]

        ## Merge back to full list
        match_keep_merge_out <- collect_files_dt[match_keep_rand_out, on=c(tissue="tissue", pheno_id="pheno_id", gene_id="gene_id",
                exp_match_direction="exp_match_direction", sample_id="sample_id")]

        ## Choose top outlier variant for each gene
        top_var_out <- match_keep_merge_out[, .(sample_id, beta, outlier, beta_check=export_col==sample(export_col, 1)),
                by=list(tissue, pheno_id, gene_id, exp_match_direction)]
	top_var_out <- top_var_out[beta_check==TRUE]
        top_var_out[, beta_check := NULL]

        ## Choose random non-outlier, matching ex_match direction

        ## Get non-outliers per pheno/gene pair
        ...
        ## Choose top non-outlier variant for each pheno, gene pair
...
## Write results
fwrite(collect_list_dt, file=paste0("get_odds_run",run,"_z_thresh.txt"), sep="\t")
